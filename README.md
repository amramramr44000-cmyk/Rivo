...
## Rivo v24.1 security hardening — server-side signup CAPTCHA

This build replaces the browser-only signup human check with a small 4–6 character image CAPTCHA backed by Supabase Edge Functions.

### One-time Supabase setup
1. Run `supabase_security_hardening.sql` in Supabase SQL Editor.
2. Deploy `supabase/functions/rivo-signup-captcha/index.ts` as `rivo-signup-captcha`.
3. Deploy `supabase/functions/rivo-signup/index.ts` as `rivo-signup`.
4. Keep `SERVICE_ROLE_KEY` available only as an Edge Function secret. Never copy it into browser code.
5. In Supabase Auth settings, turn **off** public user sign-ups / "Allow new users to sign up" after the new Edge Function works. Existing accounts can still sign in. The Edge Function creates new accounts server-side with `email_confirm: true`.
6. Deploy the updated static site.

The CAPTCHA challenge expires after two minutes, is one-time use, accepts at most five code attempts, issues a short-lived one-time verification token, and limits challenge issuance per IP window. The final account creation request validates that token server-side, so changing JavaScript in DevTools cannot bypass the check.

The UI and existing profile/social/RLS behavior are otherwise left intact.
