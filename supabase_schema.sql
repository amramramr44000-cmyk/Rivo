...
begin
  ...
  else insert into public.rivo_message_reactions(message_id,user_id,reaction) values(p_message_id,auth.uid(),'⭐') on conflict(message_id,user_id) do update set reaction='⭐',created_at=now(); end if;
...
-- ============================================================
-- Rivo Content Safety + Single Star Reaction (integrated migration)
-- ============================================================
create extension if not exists pgcrypto with schema extensions;
...
-- One reaction only: ⭐
-- ------------------------------------------------------------
update public.rivo_post_reactions set reaction='⭐' where reaction <> '⭐';
update public.rivo_message_reactions set reaction='⭐' where reaction <> '⭐';
