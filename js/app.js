  ...
  function initCallSystem() {
    ...
    async function attachAudioTrack(track) {
      ...
      if (!media) {
        ...
        active.remoteAudioEl = media;
      }

      if (active.remoteAudioTrack && active.remoteAudioTrack !== track) {
      ...
      try {
        ...
        try {
          ...
          active.remoteAudioFallbackEl = fallback;
        } catch {}
      }
    ...
    async function ensureMicrophonePublished(room) {
      ...
      try {
        publication = await lp.setMicrophoneEnabled(
          true,
        ...
        console.warn("[Rivo Calls] Microphone enable failed:", error);
      }

      // Wait for the SDK to finish its single microphone publication instead
      // of creating another track, which can cause duplicate audio/echo.
      for (let i = 0; i < 10; i++) {
    ...
    async function cleanupRemoteAudio(callState) {
      if (!callState) return;
      try { callState.remoteAudioTrack?.detach?.(callState.remoteAudioEl); } catch {}
      try { callState.remoteAudioTrack?.detach?.(); } catch {}
      for (const el of [callState.remoteAudioEl, callState.remoteAudioFallbackEl]) {
        try { el?.pause?.(); if (el) el.srcObject = null; el?.remove?.(); } catch {}
      }
      callState.remoteAudioTrack = null;
      callState.remoteAudioEl = null;
      callState.remoteAudioFallbackEl = null;
    }
    ...
    async function connectMedia() {
      ...
      room.on(LK.RoomEvent.TrackUnsubscribed, track => {
        if (track?.kind === LK.Track.Kind.Audio && active?.remoteAudioTrack === track) {
          try { track.detach(active.remoteAudioEl); } catch {}
          try { track.detach(); } catch {}
          active.remoteAudioTrack = null;
          try {
            if (active.remoteAudioEl) {
              active.remoteAudioEl.pause();
              active.remoteAudioEl.srcObject = null;
            }
          } catch {}
        } else {
    ...
    async function endCall(send = true) {
      ...
      try {
        ...
        await cleanupRemoteAudio(old);
        try { old?.micTrack?.stop?.(); } catch {}
    ...
    async function toggleMute() {
      ...
      try {
        ...
        if (nextEnabled) {
          const publication = await active.room.localParticipant.setMicrophoneEnabled(
            true,
            AUDIO_CAPTURE_OPTIONS,
            AUDIO_PUBLISH_OPTIONS()
          );
          ...
          await active.room.localParticipant.setMicrophoneEnabled(false);
        }
      } catch (error) {
...
voiceMessageSend?.addEventListener("click",async()=>{
  ...
  async function initAdmin() {
    ...
    try {
      ...
      if (!ok) {
        // UX-only 404 for unauthorized visitors. Real protection remains server-side
        // through adminStatus() and the protected admin RPCs; the page never renders tools.
        location.replace("404.html?from=admin");
        return;
      }
  ...
  function reactionSummary(post){
    const count=Number((post?.reactions||{})[POST_REACTION]||0);
    return count>0
      ? `<div class="post-reaction-summary" aria-label="${count} star reactions"><span class="post-reaction-star-mini" aria-hidden="true">★</span><b>${count}</b></div>`
  ...
  function renderPostCard(post){
    ...
    const starActive=myReaction===POST_REACTION;
    const starIcon=`<svg class="post-star-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m12 3.7 2.55 5.16 5.7.83-4.13 4.02.98 5.68L12 16.7l-5.1 2.69.97-5.68-4.12-4.02 5.69-.83L12 3.7Z"/></svg>`;
    const reactionSummaryHtml=reactionSummary(post);
    ...
    const reactButtons=`<div class="post-like-slot"><button type="button" class="post-like-button post-star-button ${starActive?'liked':''}" data-post-like aria-label="${starActive?'Remove star':'Star post'}" title="${starActive?'Remove star':'Star post'}">${starIcon}</button></div>`;
    ...
    const moderationAction = isOwner
      ...
      : `<button type="button" class="post-action post-report-action" data-post-report aria-label="Report post" title="Report post">${iconReport}</button>`;
    const actionCells = `${reactButtons ? `<div class="post-like-slot">${reactButtons}</div>` : ``}<button type="button" class="post-action" data-post-comments aria-label="Comments">${iconComment}</button>${moderationAction}<button type="button" class="post-action ${reposted?'active':''}" data-post-repost aria-label="Repost">${iconRepost}</button>`;
  ...
  function bindPostCard(card, post){
    ...
    likeBtn?.addEventListener('click',async e=>{
      ...
      try{
        ...
        const active=fresh?.my_reaction===POST_REACTION;
        await PF.reactPost(post.id, POST_REACTION);
        likeBtn.classList.toggle('liked', !active);
        likeBtn.setAttribute('aria-label', !active ? 'Remove star' : 'Star post');
        likeBtn.setAttribute('title', !active ? 'Remove star' : 'Star post');
        await refreshPostCard(post.id);
      }catch(err){notify(err.message||'Could not update star','error')}
  ...
  function communityCard(c){
    ...
    return `<article class="community-card community-row-card glass" data-community-card="${esc(c.id)}">
      ...
      <div class="community-row-copy">
        ...
        <div class="community-row-meta"><span>${Number(c.members_count||0)}/50 members</span><span>${esc(policy)}</span><span>${esc(voicePolicy)}</span>${role&&role!=='guest'?`<span class="community-role-tag ${esc(role)}">${role==='owner'?'Owner':role==='moderator'?'Moderator':'Member'}</span>`:''}</div>
      </div>
    ...
    if(!modal){
      ...
      modal.innerHTML=`<div class="modal-card glass community-create-modal">
        ...
        <form id="communityCreateForm" class="modal-form-grid">
          ...
          <div class="community-create-warning">Community limit: 50 members · Voice room limit: 5 people. 10,000 coins are deducted only when creation succeeds. The charge is done server-side and cannot be bypassed from the browser.</div>
  ...
  async function initCommunityChatPage(){
    ...
    let voicePresenceSyncBusy=false;
    ...
    const roleLabel=r=>r==='owner'?'Owner':r==='moderator'?'Moderator':'Member';
    ...
    root.innerHTML=`<div class="community-chat-header">
      ...
      <div class="community-chat-heading"><div class="community-chat-title-line"><h1>${esc(community.name)}</h1><span class="community-chat-role">${roleLabel(role())}</span></div><p>${Number(community.members_count||0)}/50 members · ${esc(currentVoicePolicyLabel())} can start voice</p></div>
      <div class="community-chat-head-actions">
        <button class="icon-btn community-voice-head-btn" id="communityVoiceHeadBtn" title="Community voice" aria-label="Community voice"><span class="community-voice-head-icon">⌁</span><i class="community-voice-head-dot"></i></button>
        <button class="icon-btn" id="communityMembersBtn" title="Members" aria-label="Members">☷</button>
        ${isOwner()?'<button class="icon-btn" id="communityManageBtn" title="Manage community" aria-label="Manage community">⚙</button>':''}
      </div>
    </div>
    <section id="communityVoiceBar" class="community-voice-bar"></section>
    <div id="communityVoiceMini" class="community-voice-mini hidden"></div>
    <div id="communityVoiceInvite" class="community-voice-invite hidden"></div>
    <div class="community-chat-layout">
      ...
      <aside id="communityMembersPanel" class="community-members-panel">
        ...
        <div class="community-permission-note"><b>Voice</b><span>Start: <strong id="communityVoicePolicyLabel">${esc(currentVoicePolicyLabel())}</strong></span><span>Join: <strong>All members</strong></span></div>
      </aside>
    </div>
    ...
    const renderMembers=async()=>{
      try{members=await PF.listCommunityMembers(id);}catch(e){members=[];notify(e.message||'Could not load members','error');}
      ...
      const memberRow=(m)=>{
        ...
        const isSelf=PF.normalizeUsername(m.username||'')===PF.normalizeUsername(me);
        const actionable=!isSelf && ((isOwner() && targetRole!=='owner') || (role()==='moderator' && targetRole==='member'));
        const voiceMuted=!!m.voice_muted;
        return `<div class="community-member-item ${isSelf?'is-self':''}" data-community-member="${esc(m.username)}">
          ${identityLink(m,profileMini(m),'community-member-avatar-link')}
          <div class="community-member-copy">${identityLink(m,`<b>${esc(m.displayName||m.username)}</b>`)}<span>${roleLabel(targetRole)}${isSelf?' · You':''}${voiceMuted?' · Voice muted':''}</span></div>
          <div class="community-member-actions">
            ${actionable?`<button class="icon-btn community-member-more" type="button" data-community-member-menu="${esc(m.username)}" aria-label="Manage ${esc(m.username)}" title="Manage member">•••</button>`:''}
          </div>
        </div>`;
      };
      list.innerHTML=groups.map(g=>{
        const rows=members.filter(m=>String(m.role||'member')===g.key);
        if(!rows.length)return '';
        return `<div class="community-member-group"><div class="community-section-label">${g.label} · ${rows.length}</div>${rows.map(memberRow).join('')}</div>`;
      }).join('');
      list.querySelectorAll('[data-community-member-menu]').forEach(b=>b.onclick=(e)=>{e.stopPropagation();openMemberActionMenu(b.dataset.communityMember);});
      list.querySelectorAll('[data-community-member]').forEach(row=>row.addEventListener('click',e=>{
        if(e.target.closest('button,a'))return;
        const u=row.dataset.communityMember;
        const m=members.find(x=>PF.normalizeUsername(x.username||'')===PF.normalizeUsername(u||''));
        if(m){openMemberActionMenu(m.username);}
      }));
      $('#communityMembersCount').textContent=`${members.length}/50`;
    };

    function openMemberActionMenu(username){
      const target=members.find(m=>PF.normalizeUsername(m.username||'')===PF.normalizeUsername(username||''));
      if(!target)return;
      const targetRole=String(target.role||'member');
      const isSelf=PF.normalizeUsername(target.username||'')===PF.normalizeUsername(me);
      const canManage=!isSelf && ((isOwner()&&targetRole!=='owner')||(role()==='moderator'&&targetRole==='member'));
      if(!canManage)return;
      document.getElementById('communityMemberActionMenu')?.remove();
      const canRole=isOwner()&&targetRole!=='owner';
      const muted=!!target.voice_muted;
      const el=document.createElement('div');
      el.id='communityMemberActionMenu';
      el.className='community-member-action-sheet';
      el.innerHTML=`<div class="community-member-action-card glass"><div class="community-member-action-head"><div><b>${esc(target.displayName||target.username)}</b><small>@${esc(target.username)}</small></div><button class="icon-btn" data-member-sheet-close>×</button></div>
        ${canRole?`<button class="community-action-item" data-member-role>${targetRole==='moderator'?'♙ Remove moderator':'♙ Make moderator'}</button>`:''}
        <button class="community-action-item" data-member-voice>${muted?'🎙 Unmute voice':'🎙 Mute voice'}</button>
        <button class="community-action-item danger" data-member-kick>⌫ Remove from community</button>
      </div>`;
      document.body.appendChild(el);
      el.addEventListener('click',e=>{if(e.target===el)el.remove();});
      el.querySelector('[data-member-sheet-close]')?.addEventListener('click',()=>el.remove());
      el.querySelector('[data-member-role]')?.addEventListener('click',async()=>{
        try{await PF.setCommunityModerator(id,target.username,targetRole!=='moderator');el.remove();await refreshCommunity();notify(targetRole==='moderator'?'Moderator role removed':'Moderator role granted','success');}
        catch(err){notify(err.message||'Could not update role','error');}
      });
      el.querySelector('[data-member-voice]')?.addEventListener('click',async()=>{
        try{await PF.moderateCommunityVoice(id,target.username,muted?'unmute_member':'mute_member');el.remove();await refreshCommunity();notify(muted?'Voice unmuted':'Voice muted','success');}
        catch(err){notify(err.message||'Could not change voice mute','error');}
      });
      el.querySelector('[data-member-kick]')?.addEventListener('click',async()=>{
        if(!confirm(`Remove @${target.username} from this community?`))return;
        try{await PF.moderateCommunityVoice(id,target.username,'kick_member');el.remove();await refreshCommunity();notify('Member removed','success');}
        catch(err){notify(err.message||'Could not remove member','error');}
      });
    }
    ...
    const renderRequests=async()=>{
      ...
      box.innerHTML=requests.length?`<div class="community-section-label">Join requests · ${requests.length}</div>${requests.map(r=>`<div class="community-request-item"><div class="community-member-copy"><b>${esc(r.displayName||r.username)}</b><span>@${esc(r.username)}</span></div><button class="btn btn-xxs btn-primary" data-accept-request="${esc(r.username)}">Accept</button><button class="btn btn-xxs btn-danger" data-decline-request="${esc(r.username)}" aria-label="Decline">×</button></div>`).join('')}`:'';
      box.querySelectorAll('[data-accept-request]').forEach(b=>b.onclick=async()=>{try{await PF.respondCommunityRequest(id,b.dataset.acceptRequest,true);await refreshCommunity();notify('Request accepted','success');}catch(e){notify(e.message||'Could not accept request','error');}});
      box.querySelectorAll('[data-decline-request]').forEach(b=>b.onclick=async()=>{try{await PF.respondCommunityRequest(id,b.dataset.declineRequest,false);await refreshCommunity();}catch(e){notify(e.message||'Could not decline request','error');}});
    };

    const participantLabel=p=>String(p?.name||p?.identity||'Member').trim()||'Member';
    const participantInitials=p=>participantLabel(p).slice(0,2).toUpperCase();
    const isLocalParticipant=p=>p===liveKitRoom?.localParticipant;
    const participantIsMuted=p=>{
      try{return p?.isMicrophoneEnabled===false;}catch{return false;}
    };
    const activeSpeakerIds=new Set();
    const participantIsSpeaking=p=>activeSpeakerIds.has(p?.identity);
    const remoteAudioState=new Map();

    const voiceParticipants=()=>liveKitRoom?[liveKitRoom.localParticipant,...liveKitRoom.remoteParticipants.values()]:[];

    const renderVoiceParticipants=()=>{
      const mini=$('#communityVoiceMini');
      if(!liveKitRoom){if(mini)mini.classList.add('hidden');return;}
      const people=voiceParticipants();
      if(mini)mini.innerHTML=`<div class="community-voice-mini-head"><div class="community-voice-mini-title"><span class="community-voice-mini-status">LIVE</span><strong>Community voice</strong><small>${people.length}/5 connected</small></div><button class="icon-btn community-voice-mini-close" id="leaveCommunityVoice" aria-label="Leave voice">×</button></div>
        <div class="community-voice-mini-people">${people.slice(0,5).map(p=>{
          const pid=String(p?.identity||'');
          const speakerMuted=remoteAudioState.get(pid)===true;
          return `<div class="community-voice-person ${participantIsMuted(p)?'muted':''} ${participantIsSpeaking(p)?'speaking':''}">
            <span class="community-voice-person-avatar">${esc(participantInitials(p))}</span>
            <span class="community-voice-person-name">${esc(participantLabel(p))}${isLocalParticipant(p)?' · You':''}</span>
            <button class="voice-person-tool ${speakerMuted?'off':''}" type="button" data-voice-output="${esc(pid)}" aria-label="${speakerMuted?'Unmute this participant':'Mute this participant locally'}" title="${speakerMuted?'Unmute their audio':'Mute their audio for you'}">${speakerMuted?'🔇':'🔊'}</button>
            <span class="voice-person-mic" title="${participantIsMuted(p)?'Microphone muted':'Microphone on'}">${participantIsMuted(p)?'🎙️':'🎤'}</span>
          </div>`;
        }).join('')}</div>
        <form class="community-voice-mini-chat" id="communityVoiceMiniChat"><input id="communityVoiceMiniInput" maxlength=2000 autocomplete="off" placeholder="Message the community…"><button class="voice-mini-chat-send" type="submit" aria-label="Send message">➤</button></form>
        <div class="community-voice-mini-actions"><button class="btn btn-primary" id="enableCommunityVoiceAudio">🔊 Audio</button><button class="btn btn-ghost" id="toggleCommunityVoiceMute">${participantIsMuted(liveKitRoom.localParticipant)?'🎤 Unmute':'🔇 Mute'}</button><button class="btn btn-danger" id="leaveCommunityVoice2">Leave</button></div>`;
      mini?.classList.remove('hidden');
      $('#leaveCommunityVoice')?.addEventListener('click',leaveVoice,{once:true});
      $('#leaveCommunityVoice2')?.addEventListener('click',leaveVoice,{once:true});
      $('#toggleCommunityVoiceMute')?.addEventListener('click',toggleVoiceMute,{once:true});
      $('#enableCommunityVoiceAudio')?.addEventListener('click',()=>enableVoiceAudio(true),{once:true});
      mini.querySelector('#communityVoiceMiniChat')?.addEventListener('submit',async ev=>{
        ev.preventDefault();
        const input=mini.querySelector('#communityVoiceMiniInput');
        const content=input?.value?.trim();
        if(!content)return;
        const send=mini.querySelector('.voice-mini-chat-send');
        try{send.disabled=true;await PF.sendCommunityMessage(id,content);input.value='';await renderMessages();}
        catch(e){notify(e.message||'Could not send message','error');}
        finally{send.disabled=false;input.focus();}
      });
      mini.querySelectorAll('[data-voice-output]').forEach(btn=>btn.addEventListener('click',()=>toggleParticipantOutput(btn.dataset.voiceOutput)));
      updateVoiceHeadButton();
    };

    const updateVoiceHeadButton=()=>{
      const btn=$('#communityVoiceHeadBtn'); if(!btn)return;
      const live=!!voiceRoom?.active;
      btn.classList.toggle('is-live',live); btn.classList.toggle('is-joined',!!liveKitRoom);
      btn.innerHTML=`<span class="community-voice-head-icon">📞</span><i class="community-voice-head-dot"></i>`;
      btn.title=live?(liveKitRoom?`In voice · ${voiceParticipants().length}/5`:'Voice is ringing · up to 5'):'Start voice';
    ...
    const renderVoiceBar=()=>{
      ...
      if(!active){
        box.className='community-voice-bar hidden';
        box.innerHTML='';
        updateVoiceHeadButton();
        return;
      }
      const joined=!!liveKitRoom;
      const people=joined?voiceParticipants().length:0;
      const starter=voiceRoom.started_by?.displayName||voiceRoom.started_by?.username||'A member';
      const moderation=canModerate();
      box.className='community-voice-bar is-active';
      box.innerHTML=`
        <div class="community-voice-copy">
          <div class="community-voice-status"><span class="community-voice-live-dot"></span><b>Voice live</b><span>${joined?`${people}/5 connected`:`${esc(starter)} started the call`}</span></div>
          <small>${joined?'You are in the community voice room.':'The call is available above the message box. Join only if you want to.'}</small>
        </div>
        <div class="community-voice-actions">
          ${joined?`<button class="btn btn-ghost" id="communityVoiceAudioBar" type="button">🔊 Audio</button><button class="btn btn-danger" id="communityVoiceLeaveBar" type="button">Leave</button>`:`<button class="btn btn-primary" id="communityVoiceJoinBar" type="button">Join</button>`}
          ${moderation?`<button class="btn btn-danger community-voice-end-btn" id="communityVoiceEndBar" type="button">End for everyone</button>`:''}
        </div>`;
      if(joined){
        $('#communityVoiceAudioBar')?.addEventListener('click',async()=>{await enableVoiceAudio(true);renderVoiceBar();},{once:true});
        $('#communityVoiceLeaveBar')?.addEventListener('click',leaveVoice,{once:true});
      }else{
        $('#communityVoiceJoinBar')?.addEventListener('click',async()=>{primeVoiceAudio();await joinVoice();},{once:true});
      }
      $('#communityVoiceEndBar')?.addEventListener('click',()=>endVoice(true),{once:true});
      updateVoiceHeadButton();
    };

    const showVoiceInvite=()=>{
      const box=$('#communityVoiceInvite');
    ...
    const refreshCommunity=async()=>{
      ...
      if(!community?.is_member){await leaveVoice();location.href='communities.html';return;}
      $('#communityVoicePolicyLabel').textContent=currentVoicePolicyLabel();
      const roleEl=$('.community-chat-role');if(roleEl)roleEl.textContent=roleLabel(role());
      await renderMembers();
      await renderRequests();
      voiceRoom=await PF.getCommunityVoice(id);
      renderVoiceBar();
      showVoiceInvite();
      if(voiceRoom?.active)startVoicePresenceWatcher();
      else if(voicePresenceTimer){clearInterval(voicePresenceTimer);voicePresenceTimer=null;}
    };

    async function stopLiveKit(silent=false){
      try{await liveKitRoom?.disconnect();}catch{}
      liveKitRoom=null; voiceJoined=false; activeSpeakerIds.clear(); remoteAudioState.clear();
      liveKitAudio.forEach(el=>{try{el.pause?.();el.srcObject=null;el.remove();}catch{}});
      liveKitAudio=[]; liveKitAudioByParticipant.clear();
      renderVoiceParticipants(); renderVoiceBar();
      if(!silent)showVoiceInvite();
    }
    ...
    const liveKitAudioByParticipant=new Map();
    ...
    async function primeVoiceAudio(){
      try{
        if(!voiceAudioContext){
          ...
          if(AC)voiceAudioContext=new AC();
        }
    ...
    async function enableVoiceAudio(showToast=false){
      ...
      renderVoiceBar();
      if(showToast){
        if(liveKitRoom?.canPlaybackAudio===false)notify('Tap Audio again to allow call sound on this phone.','error');
        else notify('Voice audio enabled','success');
      }
    }

    function attachAudioElement(track,participant){
      try{track.detach().forEach(x=>x.remove());}catch{}
      const pid=String(participant?.identity||'unknown');
      ...
      el.muted=false;
      el.volume=1;
      el.className='community-voice-audio';
      el.addEventListener('playing',()=>{voiceAudioUnlocked=true;renderVoiceBar();},{once:true});
      track.attach(el);
      const bucket=liveKitAudioByParticipant.get(pid)||[];
      bucket.push(el);
      liveKitAudioByParticipant.set(pid,bucket);
      document.body.appendChild(el);
      liveKitAudio.push(el);
      el.play().catch(()=>{});
    }

    function attachExistingAudioTracks(room){
      try{
        for(const participant of room.remoteParticipants.values()){
          for(const publication of participant.trackPublications.values()){
            if(String(publication.kind)!=='audio')continue;
            try{if(typeof publication.setSubscribed==='function' && !publication.isSubscribed)publication.setSubscribed(true);}catch{}
            if(publication.track)attachAudioElement(publication.track,participant);
          }
        }
      }catch{}
    }

    function removeParticipantAudio(participantId){
      const bucket=liveKitAudioByParticipant.get(String(participantId||''))||[];
      bucket.forEach(el=>{try{el.pause?.();el.remove();}catch{}});
      liveKitAudioByParticipant.delete(String(participantId||''));
      liveKitAudio=liveKitAudio.filter(el=>!bucket.includes(el));
    }

    function toggleParticipantOutput(participantId){
      const pid=String(participantId||'');
      const next=!remoteAudioState.get(pid);
      remoteAudioState.set(pid,next);
      (liveKitAudioByParticipant.get(pid)||[]).forEach(el=>{el.muted=next;el.volume=next?0:1;});
      renderVoiceParticipants();
    }
    ...
    async function syncVoicePresence(roomNameOverride=null, delay=0){
      ...
      if(!roomName||voicePresenceSyncBusy)return;
      voicePresenceSyncBusy=true;
      try{
        const cfg=window.RIVO_CALL_CONFIG||{};
        const session=(await window.__rivoSupabase?.auth.getSession())?.data?.session;
    ...
    function startVoicePresenceWatcher(){
      ...
      voicePresenceTimer=setInterval(()=>syncVoicePresence(),3000);
    }

    async function joinVoice(){
      if(!voiceRoom?.active){notify('There is no active voice room.','error');return;}
      if(liveKitRoom){renderVoiceParticipants();await enableVoiceAudio(true);return;}
      if(voiceJoinBusy)return;
      const LK=window.LivekitClient;
      if(!LK){notify('Live voice is not loaded. Refresh the page and try again.','error');return;}
      voiceJoinBusy=true;
      try{
        await primeVoiceAudio();
        ...
        room.on(LK.RoomEvent.TrackSubscribed,(track,publication,participant)=>{if(track.kind==='audio'){attachAudioElement(track,participant);renderVoiceParticipants();}});
        room.on(LK.RoomEvent.TrackUnsubscribed,(track,publication,participant)=>{try{track.detach().forEach(x=>x.remove());}catch{} if(participant?.identity)removeParticipantAudio(participant.identity);renderVoiceParticipants();});
        room.on(LK.RoomEvent.TrackSubscriptionFailed,(trackSid,participant)=>notify(`Could not receive audio from ${participantLabel(participant)}. Reconnecting…`,'error'));
        room.on(LK.RoomEvent.ParticipantPermissionChanged,()=>renderVoiceParticipants());
        room.on(LK.RoomEvent.ParticipantConnected,(participant)=>{renderVoiceParticipants();notify(`${participantLabel(participant)} joined voice`,'success');});
        room.on(LK.RoomEvent.ParticipantDisconnected,(participant)=>{activeSpeakerIds.delete(participant?.identity);removeParticipantAudio(participant?.identity);renderVoiceParticipants();notify(`${participantLabel(participant)} left voice`,'success');});
        room.on(LK.RoomEvent.ParticipantMetadataChanged,()=>renderVoiceParticipants());
        room.on(LK.RoomEvent.ParticipantNameChanged,()=>renderVoiceParticipants());
        room.on(LK.RoomEvent.TrackMuted,()=>renderVoiceParticipants());
        room.on(LK.RoomEvent.TrackUnmuted,()=>renderVoiceParticipants());
        room.on(LK.RoomEvent.LocalTrackPublished,()=>renderVoiceParticipants());
        room.on(LK.RoomEvent.LocalTrackUnpublished,()=>renderVoiceParticipants());
        room.on(LK.RoomEvent.ActiveSpeakersChanged,(speakers)=>{activeSpeakerIds.clear();(speakers||[]).forEach(p=>activeSpeakerIds.add(p.identity));renderVoiceParticipants();});
        room.on(LK.RoomEvent.AudioPlaybackStatusChanged,(playing)=>{
        ...
        attachExistingAudioTracks(room);
        try{await room.localParticipant.setMicrophoneEnabled(true);}catch(e){notify('Joined voice, but microphone permission was denied. You can still listen.','error');}
        startVoicePresenceWatcher();
        renderVoiceParticipants();renderVoiceBar();showVoiceInvite();
        notify('You joined the community voice room','success');
      }catch(e){await stopLiveKit(true);notify(e.message||'Could not join voice','error');}
      finally{voiceJoinBusy=false;}
    }
    ...
    async function toggleVoiceMute(){
      ...
      try{
        ...
        const muted=participantIsMuted(lp);
        // Personal microphone control is independent from the community's
        // voice-start policy. Only a server-enforced moderation mute blocks enable.
        await lp.setMicrophoneEnabled(muted);
        renderVoiceParticipants();
      }catch(e){
        const msg=String(e?.message||'');
        notify(/permission|publish|not allowed|forbidden|denied/i.test(msg)?'Your microphone is currently blocked by voice permissions or browser permission.':'Could not change microphone','error');
      }
    }
    ...
    async function startVoice(){
      ...
      try{
        primeVoiceAudio();
        if(voiceRoom?.active){return joinVoice();}
        voiceRoom=await PF.startCommunityVoice(id);
        renderVoiceBar();
        showVoiceInvite();
        await joinVoice();
      }catch(e){notify(e.message||'Could not start the voice room','error');}
    }

    async function endVoice(confirmIt=false){
      if(!canModerate()){notify('Only the owner or a moderator can end the voice room.','error');return;}
      if(confirmIt&&!confirm('End the community voice room for everyone?'))return;
      try{await PF.endCommunityVoice(id);await stopLiveKit(true);voiceRoom={active:false};renderVoiceBar();showVoiceInvite();notify('Voice room ended','success');}catch(e){notify(e.message||'Could not end voice room','error');}
    }
    ...
    $('#communityVoiceHeadBtn').onclick=async()=>{primeVoiceAudio();if(liveKitRoom){renderVoiceParticipants();await enableVoiceAudio(true);return;}if(voiceRoom?.active)await joinVoice();else await startVoice();};

    const openManage=()=>{
      const modal=$('#communityManageModal');
      modal.innerHTML=`<div class="modal-card glass community-manage-card"><div class="modal-head"><div><span class="eyebrow">COMMUNITY MANAGEMENT</span><h2>${esc(community.name)}</h2><p class="modal-muted">Owner has full control. Moderators can manage normal members and requests.</p></div><button class="icon-btn" data-close>×</button></div><div class="community-manage-grid">
        <section class="community-permission-card"><b>Voice permissions</b><label>Who can start voice<select class="field-input" id="manageVoicePolicy"><option value="everyone">Everyone in the community</option><option value="moderators">Owner & moderators</option><option value="owner">Owner only</option></select></label><p>Joining an active room is always available to community members.</p></section>
        <section class="community-permission-card"><b>Role permissions</b><div class="permission-list"><div><strong>Owner</strong><span>Manage community · assign/remove moderators · remove members · review requests · end voice</span></div><div><strong>Moderator</strong><span>Remove regular members · review requests · end voice when active</span></div><div><strong>Member</strong><span>Chat · join voice · leave voice</span></div></div></section>
        <button class="btn btn-primary" id="saveCommunityManage">Save changes</button>
      </div></div>`;
      modal.classList.add('open');
      ...
      $('#saveCommunityManage',modal).onclick=async()=>{try{await PF.setCommunityVoicePolicy(id,$('#manageVoicePolicy',modal).value);modal.classList.remove('open');await refreshCommunity();notify('Community permissions saved','success');}catch(e){notify(e.message||'Could not save permissions','error');}};
    };
    ...
    voiceStop=await PF.subscribeCommunityVoice(id,async()=>{
      try{
        ...
        const becameInactive=lastVoiceActive&&!next?.active;
        voiceRoom=next;
        renderVoiceBar();
        if(becameActive && PF.normalizeUsername(next?.started_by?.username||'')!==PF.normalizeUsername(me)){
          ...
          showVoiceInvite();
          notify('Voice call is ringing — tap Join to enter.','success');
          try{if(document.visibilityState==='hidden'&&'Notification'in window&&Notification.permission==='granted')new Notification(`${community.name} · Voice live`,{body:'A member started a voice room. Tap to join.'});}catch{}
        }
        if(becameInactive){showVoiceInvite();if(liveKitRoom){await stopLiveKit(true);notify('The community voice room ended.','success');}}
