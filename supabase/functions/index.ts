...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { error: profileError } = await admin.from("profiles").insert({
      id: auth.user.id, username, auth_email: syntheticEmail, public_data: publicData, private_data: privateData
    });
    if (profileError) {
      await admin.auth.admin.deleteUser(auth.user.id);
      if (/duplicate/i.test(profileError.message || "")) return json({ error: "That username is already taken." }, 409);
